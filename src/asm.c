...
static void emit_var_lda(Ast *ast, int size)
{
    switch(ast->type) {
    case TTYPE_PUNCT: {
        switch(ast->ival) {
        case '+': {
            if(ast->right->ctype->type == CTYPE_PTR ||\
                ast->right->ctype->type == CTYPE_ARRAY) {
                emit_var_lda(ast->left, csize(NULL));
                emit_var_lda(ast->right, csize(ast->left->ctype));
            } else if(ast->left->ctype->type == CTYPE_PTR ||\
                        ast->left->ctype->type == CTYPE_ARRAY){
                emit_var_lda(ast->right, csize(ast->left->ctype));
                emit_var_lda(ast->left, csize(NULL));
            }
            else {
                emit_var_lda(ast->left, csize(NULL));
                emit_var_lda(ast->right, csize(NULL));
            }
            emit("  BSA POP");
            emit("  STA R1");
            emit("  BSA POP");
    ...
    case AST_LVAR: {
        ...
        if(r != NULL && r->type.type == CTYPE_ARRAY) {
            ...
            emit("  BSA PUSH");
            sp--;
        } else if(r != NULL && r->type.type == CTYPE_PTR) {
            emit("  LDA BP");
            emit("  ADD %s", push_const(r->offset, NULL));
            emit("  STA R0");
            emit("  LDA R0 I");
    ...
    case AST_DEREF: {
        emit_var_lda(ast->operand, csize(NULL));
        emit("  BSA POP");
        emit("  STA R0");
        emit("  LDA R0 I");
        emit("  BSA PUSH");
    ...
    case AST_LITERAL: {
        emit("  LDA %s", push_const(ast->ival * size, NULL));
        emit("  BSA PUSH");
        sp--;
        break;
    }
...
static void emit_func_body(Ast *ast)
{
            ...
            if(ast->left->type == AST_LVAR) {
                ...
                if((r = sym_local->read(sym_local, ast->left->varname)) != NULL) {
                    ...
                    sp++;
                      ...
                      error("error");
            } else if(ast->left->type == AST_DEREF) {
                emit_var_lda(ast->left->operand, csize(NULL));
                emit("  BSA POP");
                emit("  STA R0");
                emit("  BSA POP");
                emit("  STA R0 I");
    ...
    case AST_DECL: {
        ...
        if(ast->declvar->ctype->type == CTYPE_ARRAY) {
            emit("  LDA %s", push_const(ast->declvar->ctype->len, NULL));
            emit("  CMA");
            emit("  INC");
            emit("  ADD SP");
            emit("  STA SP");
            sp -= ast->declvar->ctype->len;
            r->offset = sp + 1;
