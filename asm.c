...
void p_ast(Ast *ast)
{
    ...
    if(ast->type == AST_RETURN) {
        printf("RETURN node = %x retval = %x\n", ast, ast->retval);
        p_ast(ast->retval);
    }
    if(ast->type == AST_FUNCALL) {
        printf("FUNCALL node = %x fname = %s\n", ast, ast->fname);
    }
    if(ast->type == AST_FOR) {
        printf("FOR node = %x init = %x cond = %x step = %x body = %x\n",\
